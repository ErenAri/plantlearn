...
export function GrammarLesson({ onComplete }: Props) {
  ...
  const [questions, setQuestions] = useState<GrammarPrompt[]>(() =>
    pickQuestionsByDifficulty(GRAMMAR_PROMPTS, FIVE_QUESTION_DISTRIBUTION),
  )
  ...
  useEffect(() => {
    getAdaptiveDifficulty('grammar').then(dist => {
      setQuestions(pickQuestionsByDifficulty(GRAMMAR_PROMPTS, dist))
    })
  }, [])
