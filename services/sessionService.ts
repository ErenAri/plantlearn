...
const EMPTY_REWARDS: SessionRewards = {
  xp: 0,
  nutrients: {
    water: 0,
    sun: 0,
...
export async function completeSession(
  ...
  const hasLearningActivity = answered > 0 || input.reviewCount > 0
  ...
  if (!hasLearningActivity) {
    return {
      summary: {
        ...
        newStreak: oldStreakState.currentStreak,
        oldPlant,
        newPlant: oldPlant,
  ...
  const accuracy = input.accuracy ?? input.correct / answered
  ...
  if (input.isRecovery && input.reviewCount > 0) {
    newPlant = {
      ...newPlant,
